...
/**
 * A diagonal segment or straight segment
 *
 * @author Raffaele Florio (raffaeleflorio@protonmail.com)
 */
public final class AllSegment extends AbstractList<Point> implements Segment {
   ...
   */
  public AllSegment(final Integer x1, final Integer y1, final Integer x2, final Integer y2) {
    this(
   ...
   */
  public AllSegment(final Point source, final Point destination) {
    this(
      new FirstNonEmptyList<>(
        List.of(
          new StraightSegment(source, destination),
          new DiagonalSegment(source, destination)
        )
      )
    );
  }

  AllSegment(final List<Point> origin) {
    this.origin = origin;
  }
