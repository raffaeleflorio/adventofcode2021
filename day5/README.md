...
- An entry like `9,7 -> 7,7` covers points `9,7`, `8,7`, and `7,7`. For now, _only consider horizontal and vertical
  lines_: lines where either `x1 = x2` or `y1 = y2`.
...
number of lines which cover that point_ or `.` if no line covers that point. The top-left pair of `1s`, for example,
...
- An entry like `9,7 -> 7,9` covers points `9,7`, `8,8`, and `7,9`. Considering all lines from the above example would
  now produce the following diagram:
...
Consider all of the lines. _At how many points do at least two lines overlap_?
